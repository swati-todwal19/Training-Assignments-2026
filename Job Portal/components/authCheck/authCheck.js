...
export function showGuestPopup(actionType) {
    if (document.querySelector(".guest-popup"))
        return;
  ...
  `;
    ...
    const authBtn = popup.querySelector("#authActionBtn");
    authBtn.addEventListener("click", () => {
        window.location.href = redirectPath;
    });
}
export function requireAuth(actionType, callback) {
    if (isUserLoggedIn()) {
        callback();
    }
    else {
